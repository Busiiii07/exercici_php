<?php

echo 'Hola';
echo 'Hola', ' ', 'món';
echo '<p>Text</p>';

print 'Hola'; // retorna 1

$nom = 'Aina';
$edat = 19;
$actiu = true;

$nom = 'Bernat'; // es pot canviar
$total = $edat + 1;

echo $nom;

$x = 5;          // int
$x = 'cinc';     // ara string

$a = '10' + 5;   // 15 (int)
$b = '10' . 5;   // '105' (string)

var_dump($a, $b);

// Simples: text literal
$nom = 'Aina';
echo 'Hola $nom';

// Hola $nom

// Dobles: s'interpreta
$nom = 'Aina';
echo "Hola $nom";

// Hola Aina

$nom = 'Aina';
$punts = 8;

echo 'Hola' . $nom . ', tens ' . $punts . ' punts';  // concatenacio amb .
echo "Hola $nom, tens $punts punts";                  // interpolacio
echo "Hola ($nom), tens ($punts) punts";              // interpolacio amb claus

// Constants: el que no ha de canviar mai
define('IVA', 0,21);
const BOTIGA = 'ca la web';

echo BOTIGA; // sense $
$total = $base * (1 + IVA);

// IVA = 0.10; -> error fatal

// L'ambit de les variables
$missatge = 'Hola'; // ambit global

function saluda (){
    echo $missatge; // no la veu!
    %intern = 'Adeu'; // ambit local
}

saluda();

echo $intern; // tampoc la veu

// declare(strict_types=1);                     Primera linea de fitxer. PHP deixa de convertir tipus pel seu compte
// ini_set('display_errors', '1');              Canvia un pàrametre només per a aquesta petició.
// error_reporting(E_ALL);                      Decideix quins nivells d'error es tenen en compte ara mateix.
// setlocale() · date_default_timezone_set()    Idioma i zona horària per a aquest script.

?>

Errores:

1. el IVA està mal escrit perque en PHP define('IVA', 0,21); en realitat es define('IVA', 0.21);

2. %intern = 'Adeu'; està malament perque les variables de PHP comencen per $, per tant hauria de ser $intern = 'Adeu';

3. rcho $missatge; dintre d'una funció està malament perque $missatge està definida fora de la funció i la funció no pot accedir directament a aquesta variable.

4. echo $intern; fora de la funció està malament perque encara que corregim %intern per $intern, aquesta variable és local de la funció i no existeix fora d'ella.

5. $total = $base * (1 + IVA); està malament perque $base no està definida anteriorment.

6. IVA = 0.10; està malament perque IVA és una constant i no es pot modificar.

6 errors!!!